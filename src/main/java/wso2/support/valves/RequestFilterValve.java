...
public abstract class RequestFilterValve extends ValveBase {
    ...
    private static final Log log = LogFactory.getLog(RequestFilterValve.class);
...
//                }
    ...
    public boolean isAllowed(String property) {
        ...
        log.info("========== WSO2 Support : In the RequestFilterValve.isAllowed method.....");
        log.info("========== WSO2 Support : Allow pattern is set to : " + allow.pattern());
        log.info("========== WSO2 Support : The matching property : " + property);
        log.info("========== WSO2 Support : The pattern matches : " + allow.matcher(property).matches());

        // Check the deny patterns, if any
        if (deny != null && deny.matcher(property).matches()) {
            log.info("========== WSO2 Support : deny");
            return false;
        }

        // Check the allow patterns, if any
        if (allow != null && allow.matcher(property).matches()) {
            log.info("========== WSO2 Support : allow");
            return true;
        }

        // Allow if denies specified but not allows
        if (deny != null && allow == null) {
            log.info("========== WSO2 Support : deny allow");
            return true;
        }
        log.info("========== WSO2 Support : none");
